...
테이블 데이터, 자연어 처리(NLP) 연구 및 프로젝트를 진행하였고, 최근에는 LLM, RAG 분야에 관심이 있습니다.  
...
# 🚀 진행한 프로젝트
  ...
  2. #### 정형 테이블 데이터셋에 대한 딥러닝 기반 데이터 융합 기술 개발
      ...
      + 개요
        + 컬럼명 임베딩과 개체명 인식 기법을 통합하여 자동으로 조인 융합 가능한 테이블을 검색 및 융합하는 기법
        + 컬럼명 임베딩을 위해서 FastText 임베딩 모델을, 개체명 인식을 위해서 BERT 임베딩 모델을 사용함
        + 사용자에게 입력으로 다수의 테이블 데이터를 입력받으면, 그에 대한 출력으로 조인 가능한 테이블쌍을 랭킹을 매겨 제공함
          ![image](https://github.com/user-attachments/assets/a7a2ba75-de34-4c1d-870c-9286c5a42086)
      + 사용한 프로그래밍 언어
        + Python
      + [관련 코드 및 실행 방법](https://github.com/uos-dmlab/Structured-Data-Embedding/tree/master)
      + 프로젝트 실적
